...
function Datepicker() {
	...
	this._datepickerDiv = jQuery('<div id="datepicker_div"></div>');
}
...
jQuery.extend(Datepicker.prototype, {
	...
	_doKeyDown: function(e) {
		var inst = jQuery.datepicker._getInst(this._calId);
		if (jQuery.datepicker._datepickerShowing) {
			switch (e.keyCode) {
				case 9:  jQuery.datepicker.hideDatepicker('');
						break; // hide on tab out
				case 13: jQuery.datepicker._selectDay(inst, inst._selectedMonth, inst._selectedYear,
							jQuery('td.datepicker_daysCellOver', inst._datepickerDiv)[0]);
						return false; // don't submit the form
						break; // select the value on enter
				case 27: jQuery.datepicker.hideDatepicker(inst._get('speed'));
						break; // hide on escape
				case 33: jQuery.datepicker._adjustDate(inst,
							(e.ctrlKey ? -1 : -inst._get('stepMonths')), (e.ctrlKey ? 'Y' : 'M'));
						break; // previous month/year on page up/+ ctrl
				case 34: jQuery.datepicker._adjustDate(inst,
							(e.ctrlKey ? +1 : +inst._get('stepMonths')), (e.ctrlKey ? 'Y' : 'M'));
						break; // next month/year on page down/+ ctrl
				case 35: if (e.ctrlKey) jQuery.datepicker._clearDate(inst);
						break; // clear on ctrl+end
				case 36: if (e.ctrlKey) jQuery.datepicker._gotoToday(inst);
						break; // current on ctrl+home
				case 37: if (e.ctrlKey) jQuery.datepicker._adjustDate(inst, -1, 'D');
						break; // -1 day on ctrl+left
				case 38: if (e.ctrlKey) jQuery.datepicker._adjustDate(inst, -7, 'D');
						break; // -1 week on ctrl+up
				case 39: if (e.ctrlKey) jQuery.datepicker._adjustDate(inst, +1, 'D');
						break; // +1 day on ctrl+right
				case 40: if (e.ctrlKey) jQuery.datepicker._adjustDate(inst, +7, 'D');
						break; // +1 week on ctrl+down
			}
		}
		else if (e.keyCode == 36 && e.ctrlKey) { // display the date picker on ctrl+home
			jQuery.datepicker.showFor(this);
		}
	},
	...
	_doKeyPress: function(e) {
		var inst = jQuery.datepicker._getInst(this._calId);
		var chars = jQuery.datepicker._possibleChars(inst._get('dateFormat'));
	...
	_connectDatepicker: function(target, inst) {
		...
		if (showOn == 'button' || showOn == 'both') { // pop-up date picker when button clicked
			...
			var trigger = jQuery(buttonImageOnly ? '<img class="datepicker_trigger" src="' +
	...
	_inlineShow: function(inst) {
		...
		inst._datepickerDiv.width(numMonths * jQuery('.datepicker', inst._datepickerDiv[0]).width());
	}, 
	...
	dialogDatepicker: function(dateText, onSelect, settings, pos) {
		...
		if (jQuery.blockUI) {
			jQuery.blockUI(this._datepickerDiv);
		}
		return this;
	},
	...
	enableFor: function(inputs) {
		...
		inputs.each(function() {
			this.disabled = false;
			jQuery(this).siblings('button.datepicker_trigger').each(function() { this.disabled = false; });
			jQuery(this).siblings('img.datepicker_trigger').css({opacity: '1.0', cursor: ''});
			var $this = this;
			jQuery.datepicker._disabledInputs = jQuery.map(jQuery.datepicker._disabledInputs,
	...
	disableFor: function(inputs) {
		...
		inputs.each(function() {
			this.disabled = true;
			jQuery(this).siblings('button.datepicker_trigger').each(function() { this.disabled = true; });
			jQuery(this).siblings('img.datepicker_trigger').css({opacity: '0.5', cursor: 'default'});
			var $this = this;
			jQuery.datepicker._disabledInputs = jQuery.map(jQuery.datepicker._disabledInputs,
				function(value) { return (value == $this ? null : value); }); // delete entry
			jQuery.datepicker._disabledInputs[jQuery.datepicker._disabledInputs.length] = this;
		});
		return this;
	},
	...
	isDisabled: function(input) {
		input = (input.jquery ? input[0] : (typeof input == 'string' ? jQuery(input)[0] : input));
		for (var i = 0; i < jQuery.datepicker._disabledInputs.length; i++) {
			if (jQuery.datepicker._disabledInputs[i] == input) {
				return true;
			}
		}
		return false;
	...
	reconfigureFor: function(control, settings) {
		control = (control.jquery ? control[0] :
			(typeof control == 'string' ? jQuery(control)[0] : control));
	...
	showFor: function(control) {
		...
		if (jQuery.datepicker._lastInput == input) { // already here
			return;
		}
		if (jQuery.datepicker.isDisabled(input)) {
			return;
		}
		var inst = jQuery.datepicker._getInst(input._calId);
		...
		inst._datepickerDiv.css('position', (jQuery.datepicker._inDialog && jQuery.blockUI ?
			'static' : (isFixed ? 'fixed' : 'absolute'))).
			css('left', jQuery.datepicker._pos[0] + 'px').css('top', jQuery.datepicker._pos[1] + 'px');
		jQuery.datepicker._pos = null;
		jQuery.datepicker._showDatepicker(inst);
		return this;
	},
	...
	_showDatepicker: function(id) {
		...
		if (!inst._inline) {
			...
			var postProcess = function() {
				...
				jQuery.datepicker._afterShow(inst);
			};
	...
	_afterShow: function(inst) {
		...
		numMonths = (numMonths == null ? 1 : (typeof numMonths == 'number' ? numMonths : numMonths[1]));
		inst._datepickerDiv.width(numMonths * jQuery('.datepicker', inst._datepickerDiv[0]).width());
		if (jQuery.browser.msie && parseInt(jQuery.browser.version) < 7) { // fix IE < 7 select problems
			jQuery('#datepicker_cover').css({width: inst._datepickerDiv.width() + 4,
				height: inst._datepickerDiv.height() + 4});
		}
		// re-position on screen if necessary
		var isFixed = inst._datepickerDiv.css('position') == 'fixed';
		var pos = inst._input ? jQuery.datepicker._findPos(inst._input[0]) : null;
		var browserWidth = window.innerWidth || document.documentElement.clientWidth ||
			document.body.clientWidth;
		var browserHeight = window.innerHeight || document.documentElement.clientHeight ||
			document.body.clientHeight;
		...
		if ((inst._datepickerDiv.offset().left + inst._datepickerDiv.width() -
				(isFixed && jQuery.browser.msie ? document.documentElement.scrollLeft : 0)) >
				(browserWidth + scrollX)) {
			inst._datepickerDiv.css('left', Math.max(scrollX,
				pos[0] + (inst._input ? jQuery(inst._input[0]).width() : null) - inst._datepickerDiv.width() -
				(isFixed && jQuery.browser.opera ? document.documentElement.scrollLeft : 0)) + 'px');
		}
		// reposition date picker vertically if outside the browser window
		if ((inst._datepickerDiv.offset().top + inst._datepickerDiv.height() -
				(isFixed && jQuery.browser.msie ? document.documentElement.scrollTop : 0)) >
				(browserHeight + scrollY) ) {
			inst._datepickerDiv.css('top', Math.max(scrollY,
				pos[1] - (this._inDialog ? 0 : inst._datepickerDiv.height()) -
				(isFixed && jQuery.browser.opera ? document.documentElement.scrollTop : 0)) + 'px');
		}
	},
	...
	hideDatepicker: function(speed) {
		...
		if (this._datepickerShowing) {
			...
			if (this._inDialog) {
				...
				if (jQuery.blockUI) {
					jQuery.unblockUI();
					jQuery('body').append(this._datepickerDiv);
				}
			}
	...
	_tidyDialog: function(inst) {
		...
		jQuery('.datepicker_prompt', inst._datepickerDiv).remove();
	},
	...
	_selectDay: function(id, month, year, td) {
		if (this._hasClass(jQuery(td), 'datepicker_unselectable')) {
			return;
		}
		var inst = this._getInst(id);
		var rangeSelect = inst._get('rangeSelect');
		if (rangeSelect) {
			if (!this._stayOpen) {
				jQuery('.datepicker td').removeClass('datepicker_currentDay');
				jQuery(td).addClass('datepicker_currentDay');
			} 
			this._stayOpen = !this._stayOpen;
		}
		inst._currentDay = jQuery('a', td).html();
	...
	dateStatus: function(date, inst) {
		return jQuery.datepicker.formatDate(inst._get('dateStatus'), date, inst._get('dayNamesShort'),
...
function DatepickerInstance(settings, inline) {
	this._id = jQuery.datepicker._register(this);
	this._selectedDay = 0;
	this._selectedMonth = 0; // 0-11
	this._selectedYear = 0; // 4-digit year
	this._input = null; // The attached input field
	this._inline = inline; // True if showing inline, false if used in a popup
	this._datepickerDiv = (!inline ? jQuery.datepicker._datepickerDiv :
		jQuery('<div id="datepicker_div_' + this._id + '" class="datepicker_inline"></div>'));
...
jQuery.extend(DatepickerInstance.prototype, {
	/* Get a setting value, defaulting if necessary. */
	_get: function(name) {
		return (this._settings[name] != null ? this._settings[name] : jQuery.datepicker._defaults[name]);
	},
	...
	_setDateFromField: function(input) {
		...
		if (dates.length > 0) {
			...
			if (dates.length > 1) {
				date = jQuery.datepicker.parseDate(dateFormat, dates[1], shortYearCutoff,
			...
			try {
				date = jQuery.datepicker.parseDate(dateFormat, dates[0], shortYearCutoff,
					dayNamesShort, dayNames, monthNamesShort, monthNames) ||
					this._getDefaultDate();
			}
			catch (e) {
				jQuery.datepicker.log(e);
	...
	_generateDatepicker: function() {
		...
		for (var col = 0; col < numMonths[1]; col++) {
			...
			var calculateWeek = this._get('calculateWeek') || jQuery.datepicker.iso8601Week;
			var dateStatus = this._get('statusForDate') || jQuery.datepicker.dateStatus;
		...
		html += (showStatus ? '<div id="datepicker_status_' + this._id + 
			...
			(jQuery.browser.msie && parseInt(jQuery.browser.version) < 7 && !this._inline ? 
	...
	_formatDate: function(day, month, year) {
		...
		var date = (day ? (typeof day == 'object' ? day : new Date(year, month, day)) :
			new Date(this._currentYear, this._currentMonth, this._currentDay));
		return jQuery.datepicker.formatDate(this._get('dateFormat'), date,
...
jQuery.fn.datepicker = function(settings) {
	return this.each(function() {
		...
		for (attrName in jQuery.datepicker._defaults) {
		...
		if (nodeName == 'input') {
			...
			jQuery.datepicker._connectDatepicker(this, inst);
		} 
		else if (nodeName == 'div' || nodeName == 'span') {
			var instSettings = jQuery.extend(jQuery.extend({}, settings || {}),
				inlineSettings || {}); // clone and customise
			var inst = new DatepickerInstance(instSettings, true);
			jQuery.datepicker._inlineDatepicker(this, inst);
		}
	});
};
...
jQuery(document).ready(function() {
	...
	jQuery(document.body).append(jQuery.datepicker._datepickerDiv).
