 ...
 * fetion api class
 * 
 * 所需PHP扩展：curl, simplexml。字符编码均为UTF-8
 * http://l4ever.cn/archives/1846
 * @version 0.1
 * @author bit.kevin@gmail.com
 */
class fetion {
    
    //communicate url 不行使用221.176.31.42 具体看http://l4ever.cn/archives/1846
    ...
    public function __construct($mobileno=false, $pwd='')
    {
    	if($mobileno)
		{
			$this->guid = self::guid();
    ...
    public function setFetion($mobileno, $pwd) {
    	$this->guid = self::guid();
    	$this->mobileno = $mobileno;
    	$this->pwd = $pwd;
    }
    ...
    private function sign_in()
    {
        ...
        if( preg_match("/Set-Cookie: ssic=(.*);/", $res[0], $matches)==0 ) {
            $this->error("无法解析ssic出, 登陆失败。可能原因：用户名和密码错误！");
            return false;
        }
        $this->ssic = $matches[1];
