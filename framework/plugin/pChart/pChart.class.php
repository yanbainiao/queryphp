   ...
   function pChart($XSize,$YSize)
    {
     ...
     $this->setFontProperties("yahei.ttf",12);
    }
...
// * Fixed in 1.27 *         { $X2 = $this->GArea_X2 - 1; $Y2 = $this->GArea_Y2 - ($this->GArea_X2 - $X1); }
   ...
   /* Compute and draw the scale 五边形表格阴影*/
  ...
  /* Compute and draw the scale 四边形表格阴影*/
   function drawGridTable($LineWidth,$Mosaic=TRUE,$R=220,$G=220,$B=220,$Alpha=100)
    {
     /* Draw mosaic 
	 //Return false;
	 $this->GArea_X1=200;
     ...
     $this->DivisionHeight=60;
	 $this->DivisionWidth=95;
	 $this->GAreaXOffset=0;
	 $this->DataCount=7;
	 */
     ...
     for($i=1;$i<=$this->DivisionCount;$i++)
      {
       if ( $YPos > $this->GArea_Y1 && $YPos < $this->GArea_Y2 )
        $this->drawLine($this->GArea_X1,$YPos,$this->GArea_X2,$YPos,$R,$G,$B);
     ...
     for($i=1;$i<=$ColCount;$i++)
      {
       if ( $XPos > $this->GArea_X1 && $XPos < $this->GArea_X2 )
        $this->drawLine(floor($XPos),$this->GArea_Y1,floor($XPos),$this->GArea_Y2,$R,$G,$B);
       $XPos = $XPos + $this->DivisionWidth;
      }
    }
